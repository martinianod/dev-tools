...
test("M0 Compose defaults bind locally and remove root-equivalent sockets", async () => {
  ...
  const sonarBootstrapBlock = compose.split("  sonarqube-credential-bootstrap:")[1].split("  sonar-postgres:")[0];
  ...
  assert.match(sonarBootstrapBlock, /api\/users\/change_password/);
  ...
  assert.doesNotMatch(sonarCompose.split("  sonarqube:")[1].split("  sonarqube-credential-bootstrap:")[0], /wget/);
});
