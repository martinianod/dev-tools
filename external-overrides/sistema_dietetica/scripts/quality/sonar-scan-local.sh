...
SONAR_JAVA_BINARIES="${SONAR_JAVA_BINARIES:-$(discover_csv_paths apps/backend/target/classes)}"
SONAR_JACOCO_XML_REPORT="${SONAR_JACOCO_XML_REPORT:-$(discover_csv_paths apps/backend/target/site/jacoco/jacoco.xml)}"
SONAR_TYPESCRIPT_TSCONFIG_PATHS="${SONAR_TYPESCRIPT_TSCONFIG_PATHS:-apps/web/tsconfig.json}"

if [ -z "${SONAR_JAVA_BINARIES}" ] && find apps/backend/src/main/java -name '*.java' -print -quit | grep -q .; then
  echo "ERROR: no se detectaron clases Java compiladas. Ejecuta primero mvn verify." >&2
  exit 1
fi
...
export SONAR_JAVA_BINARIES SONAR_JACOCO_XML_REPORT SONAR_TYPESCRIPT_TSCONFIG_PATHS
...
echo "Running SonarQube analysis for sistema-dietetica"
